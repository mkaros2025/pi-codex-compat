...
import { DEFAULT_CODEX_COMPAT_CONFIG } from "../src/config.ts";
import { shouldActivate } from "../src/model.ts";

test("auto mode matches model IDs by configurable prefix", () => {
  assert.equal(shouldActivate({ id: "gpt-5.6" }, DEFAULT_CODEX_COMPAT_CONFIG), true);
  assert.equal(shouldActivate({ id: "claude-sonnet" }, DEFAULT_CODEX_COMPAT_CONFIG), false);
  assert.equal(
